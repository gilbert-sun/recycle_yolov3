...
if __name__ == '__main__':
    ...
    dataset_input1 = parameters.dataset_input
    dataset_output = parameters.dataset_output
    time_interval = args.time_interval
    ratio = args.ratio
    angle_interval = parameters.angle_interval
    show_image = parameters.show_image

    for dataset_input in os.listdir(dataset_input1):
        dirname_input_image  = os.path.join(  dataset_input1  +"/images"   )
        dirname_input_label  = os.path.join(  dataset_input1  +"/labels" )
        dirname_output_image = os.path.join( dataset_output , "images" )
        dirname_output_label = os.path.join( dataset_output , "labels" )

        print("----{}:{}:{}:{} \n".format(dirname_output_image, dirname_output_label, dirname_input_image,dirname_input_label))
